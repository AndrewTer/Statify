<?
  include("requests/user-activity-list.php");

  if ($activity_date_list = pg_fetch_row($user_activity_date_query))
  {
    $activity_list_count = pg_num_rows($user_activity_query);
?>
...
<?
      ...
      {
        $activity_list = pg_fetch_array($user_activity_query, $i);
        $activity_type = $activity_list[0];
        $creation_date = $activity_list[4];

        if ($creation_date == $activity_date_main)
        {
          switch ($activity_type) {
            case 'addFriend':
              $friend = $activity_list[1];
              new_friend_activity($user_uuid, $user_uuid, $friend, $activity_num);
              break;
            case 'profilePhotoUpdate':
              $old_photo = $activity_list[2];
              $new_photo = $activity_list[3];
              new_profile_photo_activity($user_uuid, $user_uuid, $old_photo, $new_photo, $activity_num);
              break;
            default:
              break;
          }
...
<?
    }while ($activity_date_list = pg_fetch_row($user_activity_date_query));
