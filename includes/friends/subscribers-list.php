...
<?
          $preview_photo_check = file_exists('users/'.$subscriber_uuid.'/'.get_user_avatar_preview($subscriber_uuid)) ? 1 : 0;
          if ($ban_check == 'success')
            if (!is_null(check_user_online_status($subscriber_uuid)))
              if (get_user_avatar($subscriber_uuid))
                echo '<img class="rounded-circle online m-0 p-0 pointer" width="70px" height="70px" 
                            src="users/'.$subscriber_uuid.'/'.($preview_photo_check == 1 ? get_user_avatar_preview($subscriber_uuid) : get_user_avatar($subscriber_uuid)).'" 
                            alt="'.get_user_fullname($subscriber_uuid).'" 
                            onclick="event.preventDefault();openProfilePictureModal(\''.$user_uuid.'\',\''.$subscriber_uuid.'\',\''.get_user_avatar($subscriber_uuid).'\');">';
              else
