<div id="block-received" class="p-0">
<?
$received_list = get_received_list($user_uuid);

if($received_list)
{
  for ($received_num = 0; $received_num < count($received_list); $received_num++)
  {
    $received_uuid = $received_list[$received_num];
    $received_nickname = '@'.get_user_nickname($received_uuid);

    $mutual_friends_array = get_mutual_friends_list($user_uuid, $received_uuid);
    $mutual_friends_count = ($mutual_friends_array) ? count($mutual_friends_array) : 0;

    $hash_modal = sha1($received_uuid.$received_nickname);
    $ban_check = ban_check($received_uuid);
    $premium_status = check_premium_active($received_uuid);
?>
...
?>
        ...
        <div class="w-100 m-0 p-1 d-flex flex-row received-card-header">
...
?>
          ...
          <div class="w-100 m-0 p-0">
            <div class="w-75 m-0 p-0">
              <a class="m-0 p-0" href="./?u=<?= get_user_nickname($received_uuid); ?>">
<?
            if ($premium_status)
            {
?>
...
?>
              ...
              <p class="w-100 fz-12 m-0 mb-2" id="nickname"><?= $received_nickname; ?></p>
              <a class="fz-12 m-0 mutual-friends-modal-link" onclick="event.preventDefault();openMutualFriendsModal(<?= '\''.$user_uuid.'\',\''.$received_uuid.'\''; ?>)">Общие друзья: <?= $mutual_friends_count; ?></a>
            </div>
...
<?
            ...
            {
?>
            <div class="m-0 p-0"><hr class="hr-user-info mt-2 mb-3"><?= ban_user_message($received_uuid); ?></div>
<?
            }
?>
          </div>
...
<?
  ...
  echo '<span class="d-flex justify-content-center p-5 w-100"><strong class="h5 text-center">У вас пока нет заявок в друзья</strong></span>';
?>
</div>
