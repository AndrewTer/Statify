<ul id="block-online" class="p-0">

  <div class="w-100 p-0">
    <div class="friend-card ml-0 mr-0 mt-0" id="friend-search-card">
      <div class="w-100 m-0 input-with-icon">
        <i class="fa fa-search" aria-hidden="true"></i>
        <input type="text" class="fz-14 w-100 p-1 input-field" id="onlinelist-search" placeholder="Поиск друзей">
      </div>
    </div>
  </div>

<?
  $friends_list = get_friends_list($user_uuid);

  if ($friends_list)
  {
    $online_list_num = 0;

    for ($friends_num = 0; $friends_num < count($friends_list); $friends_num++)
    {
      $friend_uuid = $friends_list[$friends_num];
      $friend_nickname = '@'.get_user_nickname($friend_uuid);

      if (!is_null(check_user_online_status($friend_uuid)))
      {
        $mutual_friends_array = get_mutual_friends_list($user_uuid, $friend_uuid);
        $mutual_friends_count = ($mutual_friends_array) ? count($mutual_friends_array) : 0;

        $hash_modal = sha1($friend_uuid.$friend_nickname);
        $ban_check = ban_check($friend_uuid);
        $premium_status = check_premium_active($friend_uuid);
?>
...
<?
          if ($ban_check == 'success')
            echo '<a class="dropdown-item pt-2 pb-2 first-item" 
                      onclick="event.preventDefault();delFriendFromFriendsList(\''.$user_uuid.'\',\''.$friend_uuid.'\', \'all\');" 
                      href="" 
                      aria-label="Убрать из друзей">Убрать из друзей</a>
                  <hr class="hr-user-info m-0">
                  <a class="dropdown-item pt-2 pb-2 last-item" 
                      onclick="event.preventDefault();openReportUserModal(\''.$user_uuid.'\',\''.$friend_uuid.'\');" 
                      href="" 
...
?>
        ...
        <div class="w-100 m-0 p-0">
          <div class="w-75 m-0 p-0">
            <a class="m-0 p-0" href="./?u=<?= get_user_nickname($friend_uuid); ?>">
<?
          if ($premium_status)
          {
?>
              <p class="username pointer text-hover search-content w-100 fz-16 d-flex align-items-center"><?= get_user_fullname($friend_uuid); ?>
...
<?
          ...
          {
?>
              <p class="username pointer text-hover w-100 fz-16 m-0 search-content"><?= get_user_fullname($friend_uuid); ?></p>
<?
          }
?> 
            </a>

            <p class="w-100 fz-12 m-0 mb-2" id="nickname"><?= $friend_nickname; ?></p>
            <a class="fz-12 m-0 mutual-friends-modal-link" onclick="event.preventDefault();openMutualFriendsModal(<?= '\''.$user_uuid.'\',\''.$friend_uuid.'\''; ?>)">Общие друзья: <?= $mutual_friends_count; ?></a>
          </div>
...
<?
          ...
          {
?>
          <div class="m-0 p-0"><hr class="hr-user-info mt-2 mb-3"><?= ban_user_message($friend_uuid); ?></div>
<?
          }
?>
        </div>
...
<?
    ...
    if ($online_list_num == 0)
      echo '<span class="d-flex justify-content-center p-5 w-100"><strong class="h5 text-center">Нет друзей в сети</strong></span>';
  }else
    echo '<span class="d-flex justify-content-center p-5 w-100"><strong class="h5 text-center">У вас пока нет друзей</strong></span>';
?>
</ul>