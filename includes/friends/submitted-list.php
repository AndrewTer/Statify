...
<?
            if ($ban_check == 'success')
              echo '<a class="dropdown-item pt-2 pb-2 first-item" href="" onclick="event.preventDefault();delRequestFromRequestList(\''.$user_uuid.'\',\''.$submitted_uuid.'\');">Отменить заявку</a>
                    <hr class="hr-user-info m-0">
                    <a class="dropdown-item pt-2 pb-2 last-item" onclick="event.preventDefault();openReportUserModal(\''.$user_uuid.'\',\''.$submitted_uuid.'\');" href="">Пожаловаться</a>';
            else
              echo '<a class="dropdown-item pt-2 pb-2 only-one-item" href="" onclick="event.preventDefault();delRequestFromRequestList(\''.$user_uuid.'\',\''.$submitted_uuid.'\');">Отменить заявку</a>';
?>
          </div>
        </div>
...
<?
          $preview_photo_check = file_exists('users/'.$submitted_uuid.'/'.get_latest_avatar_preview($submitted_uuid)) ? 1 : 0;
          if ($ban_check == 'success')
            if (!is_null(check_user_online_status($submitted_uuid)))
              if (get_latest_avatar($submitted_uuid))
                echo '<img class="rounded-circle online m-0 p-0 pointer" width="70px" height="70px"
                            src="users/'.$submitted_uuid.'/'.($preview_photo_check == 1 ? get_latest_avatar_preview($submitted_uuid) : get_latest_avatar($submitted_uuid)).'" 
                            alt="'.get_user_fullname($submitted_uuid).'" 
                            onclick="event.preventDefault();openProfilePictureModal(\''.$user_uuid.'\',\''.$submitted_uuid.'\',\''.get_latest_avatar($submitted_uuid).'\');">';
              else
...
<?
            ...
            {
?>
                <p class="username w-100 fz-16 m-0 d-flex align-items-center pointer text-hover"><?= get_user_fullname($submitted_uuid); ?>
...
<?
            ...
            {
?>
                <p class="username w-100 fz-16 m-0 pointer text-hover"><?= get_user_fullname($submitted_uuid); ?></p>
<?
            } 
?>
              </a>
