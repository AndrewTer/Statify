...
<?
      $preview_photo_check = file_exists('users/'.$search_uuid.'/'.get_user_avatar_preview($search_uuid)) ? 1 : 0;
      if ($ban_check == 'success')
        if (!is_null(check_user_online_status($search_uuid)))
          if (get_user_avatar($search_uuid))
            echo '<img class="rounded-circle online m-0 p-0 pointer" width="70px" height="70px" 
                        src="users/'.$search_uuid.'/'.($preview_photo_check == 1 ? get_user_avatar_preview($search_uuid) : get_user_avatar($search_uuid)).'" 
                        alt="'.get_user_fullname($search_uuid).'" 
                        onclick="event.preventDefault();openProfilePictureModal(\''.$user_uuid.'\',\''.$search_uuid.'\',\''.get_user_avatar($search_uuid).'\');">';
          else
...
?>
          ...
          <div class="w-100 p-0 d-flex flex-row">
            <a class="fz-12 m-0 mr-auto mutual-friends-modal-link" onclick="event.preventDefault();openMutualFriendsModal(<?= '\''.$user_uuid.'\',\''.$search_uuid.'\''; ?>)">Общие друзья: <?= $mutual_friends_count; ?></a>
<?
          switch ($friendly_user_status) {
            case 'subscriber':
              echo '<p class="m-0 ml-auto user-status-flag">Подписчик</p>';
              break;

            case 'subscribed':
              echo '<p class="m-0 ml-auto user-status-flag">Вы подписаны</p>';
              break;
