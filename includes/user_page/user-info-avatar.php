...
<?
  if ($ban_check == 'success')
    if (!is_null(check_user_online_status($current_user_uuid)))
      if (get_user_avatar($current_user_uuid))
        echo '<img class="btn-md rounded-circle online pointer"
                    id="userImg"
                    src="users/'.$current_user_uuid.'/'.get_user_avatar($current_user_uuid).'" 
                    alt="'.$user_fullname.'" 
                    onclick="event.preventDefault();openProfilePictureModal(\''.$user_uuid.'\',\''.$current_user_uuid.'\',\''.get_user_avatar($current_user_uuid).'\');">';
      else
