	...
	&& (preg_match('/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/', $_POST['friend'])))
{
	...
	$del_request_query = "DELETE FROM friendship_requests WHERE (receiver_uuid = '$user_uuid' AND author_uuid = '$friend_uuid') OR (receiver_uuid = '$friend_uuid' AND author_uuid = '$user_uuid')";
