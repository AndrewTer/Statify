/*!
 * Библиотека JavaScript jQuery v3.6.0
 * https://jquery.com/
 *
 * Включает Sizzle.js
 * https://sizzlejs.com/
...
jQuery.fn = jQuery.prototype = {
	...
	даже: функция () {
		вернуть this.pushStack (jQuery.grep (это, функция (_elem, i) {
			вернуть ( я + 1 ) % 2;
		} ) );
	},
...
setDocument = Sizzle.setDocument = функция (узел) {
	...
	содержит = hasCompare || rnative.test(docElem.contains)?
		функция ( а , б ) {
			var adown = a.nodeType === 9 ? а.documentElement : а,
				bup = b && b.parentNode;
	...
	в то время как ( пока ) {
		...
		если ( ! соответствует || ( соответствует = rcomma.exec ( soFar )) ) {
			если ( совпадение ) {
				...
				soFar = soFar.slice(match[ 0 ].length ) || уже;
			}
			groups.push((токены = []));
		}
